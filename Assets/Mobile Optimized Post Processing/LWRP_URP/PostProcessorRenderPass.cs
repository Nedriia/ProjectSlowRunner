...
public class PostProcessorRenderPass : ScriptableRendererFeature
{
    class PostProcessorPass : ScriptableRenderPass
    {
        ...
        public CommandBuffer GetBuffer(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            if(buffer == null)
            {
        ...
        private void BlurSource(CommandBuffer command, RenderTargetIdentifier source, RenderTargetIdentifier destination, float blurRadius, float resolutionDivider, int iterations, string sampleName)
        {
            ...
            {
                //blur pass
                command.SetGlobalVector(BLUR_DIR_LWRP, (i % 2 == 0) ? BlurDirection2 : BlurDirection1);
    ...
    public RenderPassEvent Order = RenderPassEvent.AfterRendering;
